...
def main():
    ...
    # Begin Threading
    myGPS = _thread.start_new_thread(GPS, ())

    mySensors = _thread.start_new_thread(readSensors, ())

    bno_thread1 = _thread.start_new_thread(send_data, ())
    bno_thread2 = _thread.start_new_thread(receive_data, ())
...
# Radio function 1 #
def send_data():
    global heading, roll, pitch, x, y, z, w
    while True:
        heading, roll, pitch = bno.euler()
        x, y, z, w = bno.quaternion()
        msg = '192.168.4.2:' + str(heading) + ',' + str(roll) + ',' + str(pitch) + ';' + str(x) + ',' + str(
            y) + ',' + str(z) + ',' + str(w) + '\n'
    ...
    while True:
        ...
        while not data:
            try:
                ...
                pass
        msg = data.decode()
        ip, command = msg.strip(':')
        g1, g2, g3, g4 = command.strip(',')
        # GLIDER DROP
        if Glider == 5:
            if g3 != '1':
                if g1 == '1':
...
def readSensors():
    ...
    global heading, roll, pitch, rollOffset, pitchOffset, x, y, z, w
    heading, roll, pitch = bno.euler()
    x, y, z, w = bno.quaternion()
